...
const getSortedChants = () => {
  ...
  // 날짜 옵션 생성 함수 - 경기가 없는 날도 포함하며 팀별로 범위 계산
  const getDateOptions = () => {
    ...
    const teamGameDates = Array.from(gameDates).sort();

    if (teamGameDates.length === 0) {
      return [
        {
          value: todayStr,
          label: `${today.getMonth() + 1}월 ${today.getDate()}일 (${['일','월','화','수','목','금','토'][today.getDay()]})`
        }
      ];
    }

    const startDate = new Date(teamGameDates[0]);
    const lastDateStr = teamGameDates[teamGameDates.length - 1];
